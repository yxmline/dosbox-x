...
int setTTFMap(int cp), FileDirExistCP(const char *name), FileDirExistUTF8(std::string &localname, const char *name);
...
bool vid_select_ttf_font_menu_callback(DOSBoxMenu* const menu, DOSBoxMenu::item* const menuitem) {
    ...
    if (lTheOpenFileName) {
        ...
        if (*name) {
            ...
            ttf_reset();
            if (!IS_PC98_ARCH) setTTFMap(dos.loaded_codepage);
#if C_PRINTER
